...
use rand::Rng;
use sdk_common::{
    grpc::{CreateSwapRequest, PaySwapRequest, RefundSwapRequest, SwapParametersRequest},
    prelude::BreezServer,
    tonic_wrap, with_connection_retry,
};
use secp256k1::musig::{
...
impl TaprootReceiveSwap {
    ...
    pub async fn create_swap_address(
        ...
        let mut client = self.breez.get_taproot_swapper_client().await;
        if let Some(mut unused_swap) = self.get_unused()? {
            ...
            let resp = with_connection_retry!(client.swap_parameters(SwapParametersRequest {}))
                .await?
                .into_inner();
        ...
        let resp = client
            .create_swap(CreateSwapRequest {
                hash: hash.clone(),
                refund_pubkey: refund_pubkey.serialize().to_vec(),
            })
            .await?
            .into_inner();
    ...
    async fn get_swap_payment(&self, swap: &FullTaprootSwapData, current_tip: u32) -> Result<()> {
        ...
        let resp = with_connection_retry!(client.pay_swap(PaySwapRequest {
    ...
    pub(crate) async fn refund_cooperative(
        ...
        for input_index in 0..tx.input.len() {
            ...
            let refund_resp = client
                .refund_swap(RefundSwapRequest {
                    address: swap_info.swap.address.clone(),
                    input_index: input_index as u32,
                    pub_nonce: our_pub_nonce.serialize().to_vec(),
                    transaction: serialized_tx.clone(),
                })
                .await?
                .into_inner();
