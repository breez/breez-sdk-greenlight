...
use gl_client::bitcoin::{blockdata::{opcodes::all::{OP_CHECKSIG, OP_CHECKSIGVERIFY, OP_CSV, OP_EQUALVERIFY, OP_HASH160}, script}, consensus::serialize, hashes::ripemd160, secp256k1::{Message, PublicKey, SecretKey}, util::{sighash::{Prevouts, SighashCache}, taproot::{LeafVersion, TapLeafHash, TaprootBuilder, TaprootSpendInfo}}, Address, PackedLockTime, SchnorrSighashType, Script, Sequence, Transaction, TxIn, TxOut, Witness, XOnlyPublicKey};
use sdk_common::tonic_wrap;
use secp256k1::musig::{MusigAggNonce, MusigKeyAggCache, MusigPartialSignature, MusigPubNonce, MusigSecRand, MusigSession};

use crate::{NodeState, OpeningFeeParams, SwapInfo, SwapStatus};

use super::{error::{ReceiveSwapError, ReceiveSwapResult}, swap::{compute_tx_fee, create_swap_keys, SwapOutput}, taproot_server::TaprootSwapperAPI};
...
impl TaprootReceiveSwap {
    pub fn new(swapper_api: Arc<dyn TaprootSwapperAPI>) -> Self {
        Self {swapper_api}
    }

    pub async fn create_swap(&self, node_state: &NodeState, opening_fee_params: OpeningFeeParams) -> ReceiveSwapResult<SwapInfo> {
        ...
        let swap_info = SwapInfo {
            ...
            lock_height: resp.lock_time,
            max_allowed_deposit: std::cmp::min(
                (node_state.max_receivable_msat / 1000) as i64,
                parameters.max_swap_amount_sat,
            ),
            max_swapper_payable: parameters.max_swap_amount_sat,
            min_allowed_deposit: parameters.min_swap_amount_sat,
            paid_msat: 0,
    ...
    pub fn create_fake_unilateral_refund_tx(
        ...
        Ok(Transaction {
            ...
            input: utxos
                ...
                .map(|utxo| {
                    Ok(TxIn {
                        witness: vec![[1; 64].to_vec(), [1; 65].to_vec(), [1; 37].to_vec()],
                        ..utxo.try_into()?
                    })
                })
                .collect::<Result<_, _>>()?,
    ...
    pub async fn create_cooperative_refund_tx(
        ...
        let weight = self.create_fake_cooperative_refund_tx(swap_info, utxos, destination_address)?.weight();
    ...
    pub fn create_unilateral_refund_tx(
        ...
        let weight = self.create_fake_unilateral_refund_tx(swap_info, utxos, destination_address)?.weight();
        let fee = compute_tx_fee(weight, sat_per_vbyte);
        let value: u64 = utxos.iter().map(|utxo| utxo.amount_sat).sum().saturating_sub(fee);
        ...
        let mut tx = Transaction {
            ...
            input: utxos
                .iter()
                .map(|utxo| TxIn {
                    sequence: Sequence::from_consensus(swap_info.lock_height),
                    ..utxo.try_into()
                })
                .collect::<Result<_, _>>()?,
            output: TxOut {
