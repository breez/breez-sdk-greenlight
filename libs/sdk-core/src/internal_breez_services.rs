...
use crate::models::{
    sanitize::*, ChannelState, ClosedChannelPaymentDetails, Config, LspAPI, NodeState, Payment,
    PaymentDetails, PaymentType, ReverseSwapPairInfo, ReverseSwapServiceAPI, SwapInfo, SwapperAPI,
...
pub(super) struct InternalBreezServices {
    config: Config,
    started: Mutex<bool>,
    ...
    event_listener: Option<Arc<Box<dyn EventListener>>>,
    backup_watcher: Arc<BackupWatcher>,
    shutdown_sender: watch::Sender<()>,
    shutdown_receiver: watch::Receiver<()>,
}

impl InternalBreezServices {
    pub async fn connect(
        req: ConnectRequest,
        event_listener: Arc<Box<dyn EventListener>>,
    ) -> BreezServicesResult<Arc<InternalBreezServices>> {
    ...
    async fn start(self: &Arc<InternalBreezServices>) -> BreezServicesResult<()> {
        let mut started = self.started.lock().await;
        ensure_sdk!(
            !*started,
    ...
    async fn start_background_tasks(self: &Arc<InternalBreezServices>) -> SdkResult<()> {
        // start the signer
    ...
    async fn start_backup_watcher(self: &Arc<InternalBreezServices>) -> Result<()> {
        self.backup_watcher
    ...
    async fn start_node_keep_alive(
        self: &Arc<InternalBreezServices>,
        shutdown_receiver: watch::Receiver<()>,
    ) {
    ...
    async fn track_backup_events(self: &Arc<InternalBreezServices>) {
        let cloned = self.clone();
        tokio::spawn(async move {
    ...
    async fn track_invoices(self: &Arc<InternalBreezServices>) {
        let cloned = self.clone();
        tokio::spawn(async move {
            let mut shutdown_receiver = cloned.shutdown_receiver.clone();
            loop {
    ...
    async fn track_logs(self: &Arc<InternalBreezServices>) {
        let cloned = self.clone();
        tokio::spawn(async move {
            let mut shutdown_receiver = cloned.shutdown_receiver.clone();
            loop {
    ...
    async fn track_new_blocks(self: &Arc<InternalBreezServices>) {
        let cloned = self.clone();
        tokio::spawn(async move {
    ...
    async fn track_swap_events(self: &Arc<InternalBreezServices>) {
        let cloned = self.clone();
        tokio::spawn(async move {
...
impl BreezServicesBuilder {
    ...
    pub async fn build(
        ...
        let breez_services = Arc::new(InternalBreezServices {
            config: self.config.clone(),
...
pub(crate) mod tests {
    ...
    use crate::internal_breez_services::{BreezServicesBuilder, InternalBreezServices};
    ...
    pub(crate) async fn breez_services() -> Result<Arc<InternalBreezServices>> {
    ...
    ) -> Result<Arc<InternalBreezServices>> {
        let node_api =
