...
impl BTCReceiveSwap {
    ...
    pub(crate) async fn refund_swap(&self, request: RefundRequest) -> Result<RefundResponse> {
        let swap_info = self
            .persister
            .get_swap_info_by_address(request.swap_address.clone())?
            .ok_or_else(|| {
                anyhow!(format!(
                    "swap address {} was not found",
                    request.swap_address
                ))
            })?;

        let transactions = self
            .chain_service
            .address_transactions(request.swap_address.clone())
            .await?;
        let utxos = get_utxos(request.swap_address, transactions)?;
...
mod tests {
    ...
    async fn test_expired_swap() -> Result<()> {
        ...
        let refund_response = swapper.refund_swap(request).await?;
        let swap = swapper
            .get_swap_info(swap_info.clone().bitcoin_address)?
            .unwrap();
        assert_eq!(swap.status, SwapStatus::Expired);
