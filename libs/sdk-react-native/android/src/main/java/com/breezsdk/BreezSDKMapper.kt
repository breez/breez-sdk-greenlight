...
fun asCurrencyInfo(currencyInfo: ReadableMap): CurrencyInfo? {
    ...
    val localizedName = if (hasNonNullKey(currencyInfo, "localizedName")) currencyInfo.getArray("localizedName")?.let { asLocalizedNameList(it) } else null
    val localeOverrides = if (hasNonNullKey(currencyInfo, "localeOverrides")) currencyInfo.getArray("localeOverrides")?.let { asLocaleOverridesList(it) } else null
    return CurrencyInfo(
        name,
        fractionSize,
        spacing,
...
fun asReverseSwapPairInfo(reverseSwapPairInfo: ReadableMap): ReverseSwapPairInfo? {
    ...
    val totalEstimatedFees = if (hasNonNullKey(reverseSwapPairInfo, "totalEstimatedFees")) reverseSwapPairInfo.getDouble("totalEstimatedFees").toULong() else null
