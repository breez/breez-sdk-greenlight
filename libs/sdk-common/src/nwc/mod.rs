use nostr_sdk::{
    nips::nip47::{Error, NostrWalletConnectURI as _NostrWalletConnectURI},
    Keys,
};
...
impl From<_NostrWalletConnectURI> for NostrWalletConnectUri {
    fn from(value: _NostrWalletConnectURI) -> Self {
        Self {
            wallet_service_public_key: value.public_key.to_hex(),
            app_public_key: Keys::new(value.secret.clone()).public_key.to_hex(),
            app_secret: value.secret.to_secret_hex(),
            relays: value
                .relays
                .into_iter()
